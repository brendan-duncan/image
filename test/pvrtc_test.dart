...
void definePvrtcTests() {
  ...
  group('PVRTC', () {
    test('encode_rgb_4bpp', () {
      ...
      new Io.File(path + '/out/pvrtc/globe_before.png')
      ...
      new Io.File(path + '/out/pvrtc/globe_after.png')
          ..createSync(recursive: true)
          ..writeAsBytesSync(encodePng(decoded));

      List<int> pvr = new PvrtcEncoder().encodePvr(image);
      new Io.File(path + '/out/pvrtc/globe.pvr')
          ..createSync(recursive: true)
          ..writeAsBytesSync(pvr);
    });

    test('encode_rgba_4bpp', () {
      List<int> bytes = new Io.File(path + '/res/png/alpha_edge.png').readAsBytesSync();
      Image image = new PngDecoder().decodeImage(bytes);

      new Io.File(path + '/out/pvrtc/alpha_before.png')
          ..createSync(recursive: true)
          ..writeAsBytesSync(encodePng(image));
      ...
      new Io.File(path + '/out/pvrtc/alpha_after.png')
          ..createSync(recursive: true)
          ..writeAsBytesSync(encodePng(decoded));

      List<int> pvr = new PvrtcEncoder().encodePvr(image);
      new Io.File(path + '/out/pvrtc/alpha.pvr')
          ..createSync(recursive: true)
          ..writeAsBytesSync(pvr);
    });
  });
  ...
  group('PVR Decode', (){
    ...
    Io.Directory dir = new Io.Directory(path + '/res/pvr');
    ...
    for (var f in files) {
      ...
      test(name, () {
        ...
        assert(img != null);
        new Io.File(path + '/out/pvrtc/pvr_$name.png')
            ..createSync(recursive: true)
