...
Image copyImageChannels(Image src, { required Image from, bool scaled = false,
  ...
  for (final frame in src.frames) {
    for (final p in frame) {
      ...
      final r = red != null ? fromPixel.getChannelNormalized(red)
          : p.rNormalized;
      final g = green != null ? fromPixel.getChannelNormalized(green)
          : p.gNormalized;
      final b = blue != null ? fromPixel.getChannelNormalized(blue)
          : p.bNormalized;
      final a = alpha != null ? fromPixel.getChannelNormalized(alpha)
          : p.aNormalized;
