/// Constant tables shared by the VP8 lossy decoder and encoder.
///
/// These are normative: every one of them is either spelled out in RFC 6386 or
/// is the exact table libwebp uses, so decoder and encoder must read the same
/// copy. See RFC 6386 for the coefficient probabilities (13.5), the mode trees
...
/// out at this pitch so a 4x4 block is four reads at a fixed distance.
const kBps = 32;

// The ten intra 4x4 prediction modes, in the order the mode tree codes them.
const bDcPred = 0;
const bTmPred = 1;
const bVePred = 2;
const bHePred = 3;
...
const bHdPred = 8;
const bHuPred = 9;
const numBModes = bHuPred + 1 - bDcPred;

// Luma16 and chroma modes reuse the first four 4x4 mode numbers; `bPred` is
// the extra luma mode that says "this macroblock is split into 4x4 blocks".
const dcPred = bDcPred;
const vPred = bVePred;
const hPred = bHePred;
const tmPred = bTmPred;
const bPred = numBModes;

// Shape of the coefficient probability table: [type][band][context][proba].
const numTypes = 4;
const numBands = 8;
const numCtx = 3;
const numProbas = 11;
...
const kCat4 = [176, 155, 140, 135];

const kCat5 = [180, 157, 141, 134, 130];

const kCat6 = [254, 254, 243, 230, 196, 177, 153, 140, 133, 130, 129];
...
// Layout of the sample cache. Luma occupies the left sixteen columns of the
// first sixteen rows, chroma the eight columns to its right:
//
//     +----+----+
//     |YYYY|UUVV|
//     |YYYY|UUVV|
...
//     |YYYY|....|
//     +----+----+
const yuvSizeEnc = kBps * 16;
const yOffEnc = 0;
const uOffEnc = 16;
const vOffEnc = 16 + 8;

// The prediction scratch area holds every candidate prediction at once: the
// four 16x16 luma modes, the four 8x8 chroma modes for U and V side by side,
// then the ten 4x4 luma modes.
const predSizeEnc = (32 + 16 + 8) * kBps;

const i16DC16 = 0 * 16 * kBps;
const i16TM16 = i16DC16 + 16;
const i16VE16 = 1 * 16 * kBps;
const i16HE16 = i16VE16 + 16;

const c8DC8 = 2 * 16 * kBps;
const c8TM8 = c8DC8 + 16;
const c8VE8 = 2 * 16 * kBps + 8 * kBps;
const c8HE8 = c8VE8 + 16;

const i4DC4 = 3 * 16 * kBps;
const i4TM4 = i4DC4 + 4;
const i4VE4 = i4DC4 + 8;
