...
List<int> xyzToRgb(num x, num y, num z) {
  ...
  y /= 100;
  z /= 100;
  num r = (3.240454836 * x) + (-1.537138850 * y) + (-0.498531547 * z);
  num g = (-0.96926639 * x) + (1.87601093 * y) + (0.041556082 * z);
  num b = (0.05564342 * x) + (-0.20402585 * y) + (1.05722516 * z);
  if (r > 0.0031308) {
...
List<int> labToRgb(num l, num a, num b) {
  ...
  // xyz to rgb
  num R = x * 3.240454836 + y * (-1.537138850) + z * (-0.498531547);
  num G = x * (-0.96926639) + y * 1.87601093 + z * 0.041556082;
  num B = x * 0.05564342 + y * (-0.20402585) + z * 1.05722516;
  ...
  return [
    (R * 255.0).clamp(0, 255).round(),
    (G * 255.0).clamp(0, 255).round(),
    (B * 255.0).clamp(0, 255).round()
  ];
}
...
List<num> rgbToLab(num r, num g, num b) {
  ...
  num x = r * 0.4124564 + g * 0.3575761 + b * 0.1804375;
  num y = r * 0.2126729 + g * 0.7151522 + b * 0.0721750;
  num z = r * 0.0193339 + g * 0.1191920 + b * 0.9503041;
